...
function ResultPair({ item, before, after, lazy = false, language }) {
  return <div className="home-v2-result-pair"><figure><img loading={lazy ? "lazy" : undefined} decoding="async" src={item.before} alt={`${before}: ${item.title[language]}`} /><figcaption>{before}</figcaption></figure><figure><img loading={lazy ? "lazy" : undefined} decoding="async" src={item.after} alt={`${after}: ${item.title[language]}`} /><figcaption>{after}</figcaption></figure></div>;
}
...
function Home() {
  ...
  return <div className="renew-home renew-home-v2">
    <HomeSectionTracking />
    <section className="home-v2-hero" id="home"><div className="home-v2-shell home-v2-hero-grid"><div><p className="home-v2-kicker">{text.hero[0]}</p><h1>{text.hero[1]}</h1><p className="home-v2-lead">{text.hero[2]}</p><div className="home-v2-hero-price"><span>{language === "nl" ? "Bank reinigen" : "Sofa cleaning"}</span><strong>{priceById.banken.price[language]}</strong></div><p className="home-v2-price-note">{text.priceNote}</p><div className="home-v2-actions"><WhatsAppButton label={text.primaryCta} tracking={{ cta_location: "hero", section: "home" }} /><a className="btn btn-secondary" href="#prijzen">{text.pricesCta}</a></div></div><figure className="home-v2-hero-media"><ResponsiveCollage priority alt="Voor en na resultaat van bankreiniging met professionele apparatuur van RenewCleaning" /></figure></div></section>
    ...
    <section className="home-v2-section home-v2-business" id="zakelijk"><div className="home-v2-shell home-v2-business-grid"><Building2 aria-hidden="true" size={32} /><div><p className="home-v2-kicker">{text.business[0]}</p><h2>{text.business[1]}</h2><p>{text.business[2]}</p></div><div className="home-v2-actions"><WhatsAppButton label={text.business[3]} tracking={{ cta_location: "business_section", section: "zakelijk" }} /><Link className="btn btn-secondary" to="/zakelijk">{text.business[4]}</Link></div></div></section>
    ...
    <section className="home-v2-contact" id="contact"><div className="home-v2-shell"><div className="home-v2-contact-intro"><div><p className="home-v2-kicker">{text.contact[0]}</p><h2>{text.contact[1]}</h2><p>{text.contact[2]}</p></div><WhatsAppButton label={text.primaryCta} tracking={{ cta_location: "contact_section", section: "contact" }} /></div><div className="home-v2-contact-grid"><ol>{text.contactSteps.map((step, index) => <li key={step}><span>{String(index + 1).padStart(2, "0")}</span>{step}</li>)}</ol><div className="home-v2-contact-methods"><TrackedContactLink eventName="whatsapp_click" tracking={{ cta_location: "contact_panel", section: "contact", cta_text: "WhatsApp" }} href={company.whatsappHref} target="_blank" rel="noreferrer"><MessageCircle aria-hidden="true" size={21} /><span><strong>WhatsApp</strong><small>{text.whatsappHelp}</small></span></TrackedContactLink><TrackedContactLink eventName="phone_click" tracking={{ cta_location: "contact_panel", section: "contact" }} href={company.phoneHref}><Phone aria-hidden="true" size={21} /><span><strong>{company.phoneDisplay}</strong><small>{text.callHelp}</small></span></TrackedContactLink><TrackedContactLink eventName="email_click" tracking={{ cta_location: "contact_panel", section: "contact" }} href={`mailto:${company.email}`}><Mail aria-hidden="true" size={21} /><span><strong>{company.email}</strong><small>{text.mailHelp}</small></span></TrackedContactLink></div></div><div className="home-v2-socials"><div><p className="home-v2-kicker">{language === "nl" ? "Volg RenewCleaning" : "Follow RenewCleaning"}</p><h2>{text.contact[3]}</h2><p>{text.contact[4]}</p></div><div className="home-v2-social-grid">{socials.map((social) => { const Icon = socialIcons[social.icon]; const card = <><Icon aria-hidden="true" size={22} /><span><strong>{social.label}</strong><small>{social.handle}</small></span></>; return social.url ? <TrackedContactLink key={social.id} eventName="social_click" tracking={{ section: "contact", platform: social.id }} href={social.url} target="_blank" rel="noreferrer" aria-label={`${social.label}: ${social.handle}`}>{card}</TrackedContactLink> : <div key={social.id} aria-label={`${social.label}: ${social.handle}`}>{card}</div>; })}</div></div></div></section>
  </div>;
}
