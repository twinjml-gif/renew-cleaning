...
const pageContent = {
  nl: {
    ...
    services: [
      ["banken", "Banken reinigen", "Professionele dieptereiniging van stoffen banken, hoekbanken en loungesets. We behandelen vuil en vlekken in het textiel zorgvuldig."],
      ["matras", "Matrassen reinigen", "Grondige textielreiniging om uw matras op te frissen, met aandacht voor materiaal en gebruik."],
      ["tapijt", "Tapijt reinigen", "Dieptereiniging van tapijt en vloerkleden, afgestemd op de vezel, afmeting en vervuiling."],
      ["kinderwagen", "Kinderwagens reinigen", "Grondige reiniging van de textiele delen en bereikbare onderdelen van uw kinderwagen."],
      ["stoelen", "Eetkamerstoelen reinigen", "Een nette reiniging van gestoffeerde eetkamerstoelen. De prijs geldt per eetkamerstoel."],
      ["other-textile", "Andere textiele meubels", "Voor fauteuils, poefs, eetkamerbanken en andere gestoffeerde of textiele meubels bekijken we uw foto's eerst zorgvuldig."],
    ],
  ...
  en: {
    ...
    services: [
      ["banken", "Sofa cleaning", "Professional deep cleaning for fabric sofas, corner sofas and lounge sets. We treat dirt and stains in the textile carefully."],
      ["matras", "Mattress cleaning", "Thorough textile cleaning to freshen your mattress, with attention to material and use."],
      ["tapijt", "Carpet cleaning", "Deep cleaning for carpets and rugs, tailored to fibre, size and level of soiling."],
      ["kinderwagen", "Stroller cleaning", "Thorough cleaning of textile parts and reachable elements of your stroller."],
      ["stoelen", "Dining chair cleaning", "Careful cleaning for upholstered dining chairs. The price applies to each dining chair."],
      ["other-textile", "Other textile furniture", "For armchairs, pouffes, dining benches and other upholstered or textile furniture, we first review your photos carefully."],
    ],
...
function Services() {
  ...
  return (
    <div className="renew-services">
      <section className="services-hero"><div className="services-shell services-hero-grid"><div><p className="services-kicker">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="services-lead">{copy.intro}</p><div className="services-actions"><WhatsAppButton label={copy.primaryCta} /><Link className="btn btn-secondary" to="/prijzen">{copy.pricesCta}</Link></div></div><figure className="services-hero-image"><ResponsiveCollage alt="Voor en na resultaat van bankreiniging met professionele apparatuur van RenewCleaning" /></figure></div></section>
      ...
      <section className="services-section"><div className="services-shell"><header className="services-heading"><p className="services-kicker">{copy.eyebrow}</p><h2>{copy.servicesTitle}</h2><p>{copy.servicesText}</p></header><div className="services-grid">{copy.services.map(([priceId, title, text]) => <article className="services-card" key={priceId}><Sparkles aria-hidden="true" size={20} /><h3>{title}</h3><strong>{priceById[priceId].price[language]}</strong><p>{text}</p><WhatsAppButton className="services-card-cta" label={copy.primaryCta} /></article>)}</div></div></section>
